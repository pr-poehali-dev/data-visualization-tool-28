...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if existing:
            ...
            if role != ADMIN_ROLE:
                cur.execute("UPDATE users SET role = %s WHERE id = %s", (ADMIN_ROLE, user_id))
                conn.commit()
            return {
                "statusCode": 200,
                "headers": {"Access-Control-Allow-Origin": "*", "Content-Type": "application/json"},
                "body": json.dumps({"ok": True, "message": f"Пользователь уже существует (id={user_id}), роль обновлена до {ADMIN_ROLE}"}),
                "isBase64Encoded": False,
            }

        pw_hash = hash_password(ADMIN_PASSWORD)
        cur.execute(
            "INSERT INTO users (email, password_hash, name, role, status, subscription) VALUES (%s, %s, %s, %s, 'active', 'premium') RETURNING id",
            (ADMIN_EMAIL, pw_hash, ADMIN_NAME, ADMIN_ROLE)
        )
