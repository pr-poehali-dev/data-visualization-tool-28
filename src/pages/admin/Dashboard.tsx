...
const ADMIN_URL = "https://functions.poehali.dev/ADMIN_PLACEHOLDER"
...
export default function Dashboard() {
  ...
  const metrics = [
    { label: "Всего пользователей", value: stats.total_users.toLocaleString(), icon: "Users", color: "text-blue-400", sub: `+${stats.new_month} за месяц` },
    { label: "Онлайн сейчас", value: stats.online_now.toString(), icon: "Wifi", color: "text-green-400", sub: "активных сессий" },
    { label: "Новых сегодня", value: stats.new_today.toString(), icon: "UserPlus", color: "text-primary", sub: `${stats.new_week} за неделю` },
    { label: "Выручка за месяц", value: `${stats.revenue_month.toLocaleString()} ₽`, icon: "TrendingUp", color: "text-yellow-400", sub: `${stats.paid_orders} оплат` },
    { label: "Общая выручка", value: `${stats.total_revenue.toLocaleString()} ₽`, icon: "DollarSign", color: "text-purple-400", sub: "за всё время" },
    { label: "Открытых тикетов", value: stats.open_tickets.toString(), icon: "LifeBuoy", color: "text-orange-400", sub: "требуют ответа" },
  ]
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent>
            <div className="space-y-4">
              {[
                { label: "Сегодня", value: stats.new_today, max: 100 },
                { label: "За неделю", value: stats.new_week, max: 500 },
                { label: "За месяц", value: stats.new_month, max: 1000 },
              ].map((r) => (
                <div key={r.label}>
            ...
            <div className="mt-6 pt-4 border-t border-border">
              <div className="grid grid-cols-3 gap-4 text-center">
                {[
                  { label: "DAU", value: stats.new_today },
                  { label: "WAU", value: stats.new_week },
                  { label: "MAU", value: stats.new_month },
                ].map((m) => (
                  <div key={m.label}>
